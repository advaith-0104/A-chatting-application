...
# as an environment variable (FIREBASE_SERVICE_ACCOUNT_KEY_CONTENT).
# This is more secure than including the file directly in the repository.
...
if not FIREBASE_SERVICE_ACCOUNT_KEY_CONTENT:
    ...
    # For now, we'll let it proceed, but Firebase initialization will fail.
    # sys.exit(1) # Do not exit here, let Render handle restarts
else:
    try:
        # Initialize Firebase Admin SDK using the content from the environment variable
...
# This is crucial for consistent handling and avoiding 'Timestamp' attribute errors
def convert_firestore_timestamp(obj):
    if isinstance(obj, firestore.Timestamp):
    ...
    try:
        ...
        for req_doc in incoming_query:
            ...
            incoming_requests.append({
                ...
                "timestamp": convert_firestore_timestamp(req_data['timestamp']) # Use helper
            })
...
def get_chat_history(user1_id, user2_id):
    """
    Retrieves chat messages between two users from Firestore and formats timestamps.
    """
    ...
    try:
        ...
        for msg_doc in messages_ref:
            ...
            # Convert Firestore Timestamp to string for frontend display using helper
